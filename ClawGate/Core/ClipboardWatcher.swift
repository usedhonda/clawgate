...
final class ClipboardWatcher {
    ...
    private var emittedChangeCounts: Set<Int> = []
    ...
    func start() {
        timer?.invalidate()
        lastChangeCount = NSPasteboard.general.changeCount
        ownedChangeCounts.removeAll()
        emittedChangeCounts.removeAll()
    ...
    /// Register the exact pasteboard transaction generated by ClawGate.
    /// The watcher consumes that change once and never treats it as a user offer.
    func registerOwnedChangeCount(_ changeCount: Int) {
        ownedChangeCounts.insert(changeCount)
    }

    func registerOwnedCurrentChange() {
        registerOwnedChangeCount(NSPasteboard.general.changeCount)
    }
    ...
    private func check() {
        ...
        if ownedChangeCounts.remove(current) != nil { return }
        guard emittedChangeCounts.insert(current).inserted else { return }
