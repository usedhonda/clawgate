...
# Tracks local client events and whether matching events appear on Host A server.
#
# Usage:
...
while true; do
  ...
  local_poll="$(curl -fsS -m 3 "$LOCAL_API/v1/poll?since=$client_cursor" 2>/dev/null || echo '')"
  ...
  printf '[%s] client_cursor=%s server_cursor=%s pending=%s\n' "$(date '+%H:%M:%S')" "$client_cursor" "$server_cursor" "$pending_now" | tee -a "$LOG_FILE"
