...
export default {
  ...
  firstTime: [
    ...
    "- AUTONOMOUS: Your core role is facilitator between CC and Cdx (the workers). Code quality checks matter, but your top priority is keeping work flowing. Common stalls: a worker forgets to report back to the requester / a requester waits endlessly without following up. Reminding both sides is your key responsibility. Stay in character per SOUL.md.",
    ...
    "  Telegram is the default destination for CC/Cdx development messaging. LINE is reserved for normal secretary workflows; do not send development-session chatter there.",
    "  Telegram updates are milestone-based only: blocking risk, interaction-pending advisories, and final wrap-up.",
  ...
  completion: {
    ...
    autonomous: [
      ...
      "Finally — as facilitator, check these two things before closing your reply:",
      "(1) Report reminder: If the pane shows a [from:X.cc] or [from:X.cdx] request and no `tproj-msg` reply to the requester is visible, append to <cc_task>: 'You haven\\'t reported back to X.cc yet. Whether mid-work, done, or blocked — send `tproj-msg X.cc \\\"...\\\"` with your status.' Skip if reply is visible or FYI/no-reply-needed is explicit.\n(2) Follow-up reminder: If this session sent `tproj-msg Y.cdx` or `tproj-msg Y.cc` and no [from:Y.cdx]/[from:Y.cc] reply is visible, append to <cc_task>: 'No reply from Y yet. Try `tproj-msg --status Y.cdx` or follow up.' Skip if reply is already visible.",
    ],
    observe: [
  ...
  question: {
    ...
    autonomous: [
      ...
      "In autonomous mode, question events should send recommendation text to the messenger (advisor-only, no execution).",
  ...
  questionBody: {
    auto: '[{label} {project}] Claude Code is asking a question:\n\n{questionText}\n\nOptions:\n{numberedOptions}\n\n[To answer, include <cc_answer project="{project}">{option number}</cc_answer> in your reply. Use 1-based numbering (1 = first option). Text outside the tag goes to Telegram.]',
    default: "[{label} {project}] Claude Code is asking a question:\n\n{questionText}\n\nOptions:\n{numberedOptions}\n\n[Analyze the options and send your recommendation to the user via Telegram. Do NOT use <cc_answer>.]",
  },

  // ── Roster footer hints ─────────────────────────────────────────
  rosterFooter: {
    taskHint:
      "\nYou can send tasks to autonomous projects by including <cc_task>your task</cc_task> in your reply. Text outside the tags goes to Telegram.",
    sendTelegramHint:
      "\nDevelopment-session text already goes to Telegram by default. <send_telegram>text</send_telegram> remains available if you want to be explicit.",
    answerHint:
